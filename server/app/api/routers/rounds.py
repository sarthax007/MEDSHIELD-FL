...
@router.post("/", response_model=TrainingRoundResponse, status_code=status.HTTP_201_CREATED)
def trigger_round(
    ...
    existing = db.query(models.TrainingRound).filter(models.TrainingRound.round_number == round_in.round_number).first()
    if existing:
