...
from app.db.models import User, Hospital, TrainingRound, ModelVersion, AuditLog
...
def setup_db():
    ...
    model_v = ModelVersion(
        version_tag="v1.0", training_round_id=t_round.id, s3_path="s3://test/model.pth"
    )
    db.add(model_v)
...
def test_prediction_valid_image(setup_db):
    ...
    response = client.post(
        "/predictions/", headers={"Authorization": f"Bearer {token}"}, files=files
    )
    ...
    # Verify AuditLog
    db = TestingSessionLocal()
    audit_logs = db.query(AuditLog).filter(AuditLog.action == "create_prediction").all()
    assert len(audit_logs) > 0
    assert audit_logs[-1].details["predicted_class"] == data["predicted_class"]
    db.close()
