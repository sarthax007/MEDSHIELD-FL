...
from app.db.models import User, Hospital, TrainingRound, AuditLog
...
def test_trigger_round_admin(setup_db):
    ...
    # Verify AuditLog
    db = TestingSessionLocal()
    audit_logs = db.query(AuditLog).filter(AuditLog.action == "trigger_round").all()
    assert len(audit_logs) > 0
    assert audit_logs[-1].details["round_number"] == 2
    db.close()
