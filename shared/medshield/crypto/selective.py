...
class SelectiveUpdate:
    ...
    def deserialize(cls, data: bytes, public_context: ts.Context) -> "SelectiveUpdate":
        ...
        encrypted_critical = [
            ts.ckks_vector_from(public_context, vec_bytes) for vec_bytes in serialized_critical
        ]

        return cls(
            encrypted_critical=encrypted_critical, plaintext_non_critical=plaintext_non_critical
        )
...
def create_selective_update(model: nn.Module, public_context: ts.Context) -> SelectiveUpdate:
    ...
    return SelectiveUpdate(
        encrypted_critical=encrypted_critical, plaintext_non_critical=non_critical_vec
    )


def apply_selective_update(
    update: SelectiveUpdate, model: nn.Module, secret_context: ts.Context
) -> None:
