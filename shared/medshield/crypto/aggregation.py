...
def multiply_encrypted_vector_by_scalar(
    vector: list[ts.CKKSVector], scalar: float
) -> list[ts.CKKSVector]:
    """
...
def multiply_selective_update_by_scalar(update: SelectiveUpdate, scalar: float) -> SelectiveUpdate:
    ...
    return SelectiveUpdate(
        encrypted_critical=scaled_encrypted, plaintext_non_critical=scaled_plaintext
    )
