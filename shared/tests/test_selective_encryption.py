...
def test_selective_encryption_reassembly_and_size():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"[Test Selective Encryption] Selective payload serialized size: {selective_size_bytes / 1024 / 1024:.2f} MB")
        ...
        assert torch.max(torch.abs(original_logits - fresh_logits_before)).item() > 1e-4, "Fresh model should differ"
